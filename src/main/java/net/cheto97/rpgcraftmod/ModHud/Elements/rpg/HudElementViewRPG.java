...
public class HudElementViewRPG extends HudElementViewVanilla {
    double life;
    double maxLife;
    double armor;
    int lvl;
    ...
    public void drawElement(Gui gui, PoseStack ms, float zLevel, float partialTicks, int scaledWidth, int scaledHeight) {
        ...
        if(focused != null) {
            ...
            if(settings.getBoolValue(Settings.show_numbers_life)){
                focused.getCapability(LifeProvider.ENTITY_LIFE).ifPresent(hp ->{
                    maxLife = hp.getMax();
                    life = hp.get();
                });
                String stringLife = life + "/" + maxLife;
            ...
            if(settings.getBoolValue(Settings.show_entity_armor)) {
                focused.getCapability(DefenseProvider.ENTITY_DEFENSE).ifPresent(def ->{
                    armor = def.get();
                });
                focused.getCapability(CustomLevelProvider.ENTITY_CUSTOMLEVEL).ifPresent(level ->{
                    lvl = level.get();
                });
                if(armor > 0) {
                    String value = String.valueOf(armor);
                    String levelValue = String.valueOf(lvl);
                    bind(DAMAGE_INDICATOR);
                    gui.blit(ms, posX - 26, posY+44, 0, 36, 19, 8);
                    bind(Gui.GUI_ICONS_LOCATION);
                    ms.scale(0.5f, 0.5f, 0.5f);
                    gui.blit(ms, (posX - 24) * 2 -1, (posY + 45) * 2, 34, 9, 9, 9);
                    this.drawStringWithBackground(ms,value + "      "+"Level: "+levelValue, (posX - 18) * 2 -2, (posY + 45) * 2 + 1, -1, 0);
                    ms.scale(2f, 2f, 2f);
                }
            }
        }
