...
public class VoltWingsModel<T extends LivingEntity> extends WingsModel<T> {
    private final ModelPart modelItem;
    public VoltWingsModel(ModelPart part){
        super(part);
        this.modelItem = part;
    }
    public static LayerDefinition createLayer() {
        ResourceLocation modelLocation = new ResourceLocation(RpgcraftMod.MOD_ID,"models/item/volt_wings.json");
        MeshDefinition mesh = new MeshDefinition();
        PartDefinition part = mesh.getRoot();

        try (Reader reader = new InputStreamReader(Objects.requireNonNull(VoltWingsModel.class.getResourceAsStream("/assets/" + modelLocation.getNamespace() + "/" + modelLocation.getPath())))) {
            Gson gson = new Gson();
