...
public class DiscordsWingsModel<T extends LivingEntity> extends WingEntityModel<T> {
    ...
    public static LayerDefinition getLayerDefinition() {
        ...
        modelPartData4.addOrReplaceChild("leftWing05", CubeListBuilder.create().texOffs(32, 31)
                 ...
                 PartPose.rotation(-0.5F, 4.8F, -0.2F));
        ...
        modelPartData14.addOrReplaceChild("rightStrutLowestLeather01", CubeListBuilder.create().texOffs(79, 28)
                 ...
                 PartPose.rotation(0.5F, 0.0F, 0.0F));
    ...
    public void setupAnim(LivingEntity entity, float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch) {
        ...
        if(state == State.IDLE || state == State.CROUCHING)
            leftWing03.yRot = (float) Math.toRadians(-60);
        if(state == State.FLYING)
            leftWing03.yRot = (float) Math.toRadians(-32.5);
        rightWing03.yRot = leftWing03.yRot;
    }
}
