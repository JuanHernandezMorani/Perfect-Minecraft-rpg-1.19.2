...
public abstract class RegistryMapConfig<X, Y> extends AbstractConfigValue<ImmutableMap<X, Y>> {
    ...
    public void setToDefault() {
        ...
        this.defaultValues.forEach((x, y) -> this.rawValues.put(Objects.requireNonNull(this.registry.getKey(x)).toString(), y));
    }
    ...
    public void deserialize(JsonObject jsonObject) {
        ...
        if (jsonElement == null) {
            this.setToDefault();
            RpgcraftMod.LOGGER.error("Error while loading the config value '{}', using default values instead", getIdentifier());
            return;
        }
        rawValues = GSON.fromJson(jsonElement, this.getType());
        if (rawValues == null) {
            this.setToDefault();
            RpgcraftMod.LOGGER.error("Error while parsing the config value '{}', using default values instead", getIdentifier());
            return;
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (!isValueValid(value)) {
                RpgcraftMod.LOGGER.warn("Removing '{}' because the value '{}' is not valid", entry.getKey(), entry.getValue());
                iterator.remove();
                continue;
            }
            ...
            if (entry.getKey().contains("*")) {
                ...
                RpgcraftMod.LOGGER.warn("Found wildcard with invalid modid '{}' in '{}'", entry.getKey(), getIdentifier());
            }
            ...
            if (entry.getKey().startsWith("#")) {
                ...
                if (tagKey != null) {
                    ...
                    addAllEntries(parsedValues, entries, value);
                    RpgcraftMod.LOGGER.info("Found tag '{}' in '{}'", tagKey.location(), getIdentifier());
                    continue;
                }
                RpgcraftMod.LOGGER.warn("Found tag with invalid name '{}' in '{}'", entry.getKey(), getIdentifier());
            }
            ...
            if (loc != null && registry.containsKey(loc)) {
                ...
                RpgcraftMod.LOGGER.warn("Ignoring value '{}' from '{}' because it is not allowed", loc, getIdentifier());
            }

            //else key is invalid, remove it
            RpgcraftMod.LOGGER.warn("Found invalid entry '{}' in '{}'", entry.getKey(), getIdentifier());
            iterator.remove();
        }
    ...
    public void addAllEntries(Map<X, Y> parsedValues, List<X> entries, Y value) {
        entries.forEach((key) -> {
            if (isKeyValid(key, registry.getKey(key))) {
                ...
                RpgcraftMod.LOGGER.warn("Ignoring value '{}' from '{}' because it is not allowed", key, registry.getKey(key).toString());
            }
        });
    }
